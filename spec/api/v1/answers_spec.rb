  ...
  describe 'POST /create' do
    ...
    context 'valid attributes' do
      ...
      it 'created and saved in db' do
        expect { post api_v1_answers_path, question_id: question.id, answer: attributes_for(:answer),
                      format: :json, access_token: access_token.token }.to change(Answer, :count).by(1)

      end

      it 'return status created(201)' do
        post api_v1_answers_path, question_id: question.id, answer: attributes_for(:answer),
    ...
    context 'invalid attributes' do
      ...
      it ' not save on db' do
        expect { post api_v1_answers_path, question_id: question.id, answer: attributes_for(:invalid_answer),
                      format: :json, access_token: access_token.token }.to_not change(Answer, :count)
      end

      it 'return status 422' do
        post api_v1_answers_path, question_id: question.id, answer: attributes_for(:invalid_answer),
             format: :json, access_token: access_token.token
